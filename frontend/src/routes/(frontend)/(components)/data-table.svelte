<script lang="ts">
	...
	const columns = table.createColumns([
		...
		table.column({
			...
			header: '',
			cell: ({ value }) => {
				return createRender(DataTableActions, { address: value });
			},
			plugins: {
				sort: {
					disable: true
			...
			<Table.Body {...$tableBodyAttrs}>
				{#each $pageRows as row (row.id)}
					<Subscribe rowAttrs={row.attrs()} let:rowAttrs>
						<Table.Row {...rowAttrs} data-state={$selectedDataIds[row.id] && 'selected'} on:click={() => document.location = `/${row.original.address}/rules`}>
